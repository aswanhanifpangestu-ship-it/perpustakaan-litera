...
<table class="main">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center">{{ $u->peminjaman_count }}</td>
            <td class="text-center">{{ $u->dikembalikan_count }}</td>
            <td class="text-right">
                @if($u->total_denda > 0)
                    <span class="badge-denda">Rp {{ number_format($u->total_denda, 0, ',', '.') }}</span>
                @else
