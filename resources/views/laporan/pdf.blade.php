...
<head>
    <meta charset="UTF-8">
    <title>Laporan Denda {{ $bulanNama }} {{ $tahun }}</title>
    <style>
...
<div class="header">
    ...
    <p>Periode: {{ $bulanNama }} {{ $tahun }} &nbsp;|&nbsp; Dicetak: {{ now()->format('d M Y H:i') }}</p>
</div>

{{-- Summary --}}
<div class="summary">
    <div class="summary-box">
        <div class="val">{{ $data['totalPeminjaman'] }}</div>
        <div class="lbl">Total Peminjaman</div>
    </div>
    <div class="summary-box danger">
        <div class="val">{{ $data['totalTerlambat'] }}</div>
        <div class="lbl">Kasus Terlambat</div>
    </div>
    <div class="summary-box danger">
        <div class="val">Rp {{ number_format($data['belumBayar'], 0, ',', '.') }}</div>
        <div class="lbl">Denda Belum Bayar</div>
    </div>
    <div class="summary-box success">
        <div class="val">Rp {{ number_format($data['sudahBayar'], 0, ',', '.') }}</div>
...
<table class="main">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-center text-danger">{{ $p->jumlah_hari_terlambat }} hr</td>
            <td class="text-right">Rp {{ number_format($p->buku->denda_per_hari ?? 0, 0, ',', '.') }}</td>
            <td class="text-right text-danger">Rp {{ number_format($p->total_denda, 0, ',', '.') }}</td>
        ...
        @if($data['detailDenda']->count() > 0)
        <tr class="total-row">
            <td colspan="7" class="text-right">TOTAL DENDA:</td>
            <td class="text-right text-danger">Rp {{ number_format($data['totalDenda'], 0, ',', '.') }}</td>
            <td></td>
        </tr>
        @endif
    </tbody>
