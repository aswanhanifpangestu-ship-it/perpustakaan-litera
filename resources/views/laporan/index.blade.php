...
@section('page-title', '📊 Laporan Denda Bulanan')
...
<div class="card table-card mb-4">
    <div class="card-body py-3">
        <form action="{{ route('laporan.index') }}" method="GET" class="d-flex gap-2 align-items-center flex-wrap">
        ...
        <div class="row g-3">
            <div class="col-md-4">
                <div class="rounded-3 p-3 h-100 d-flex flex-column" style="border:1px solid #ccfbf1;background:#f0fdf9">
                    ...
                    <a href="{{ route('laporan.cetakBuku') }}" class="btn btn-sm btn-primary mt-auto" target="_blank">
                        📄 Cetak PDF Buku
                    </a>
                </div>
            </div>
            <div class="col-md-4">
                <div class="rounded-3 p-3 h-100 d-flex flex-column" style="border:1px solid #fef3c7;background:#fffbeb">
                    ...
                    <a href="{{ route('laporan.cetakPetugas') }}" class="btn btn-sm btn-warning text-dark mt-auto" target="_blank">
                        📄 Cetak PDF Petugas
                    </a>
                </div>
            </div>
            <div class="col-md-4">
                <div class="rounded-3 p-3 h-100 d-flex flex-column" style="border:1px solid #ede9fe;background:#faf5ff">
                    ...
                    <a href="{{ route('laporan.cetakUser') }}" class="btn btn-sm mt-auto" target="_blank"
        ...
        <div class="row g-3">
            <div class="col-md-6">
                <div class="rounded-3 p-3 h-100 d-flex flex-column" style="border:1px solid #ccfbf1;background:#f0fdf9">
                    ...
                    <div class="d-flex gap-2 mt-auto flex-wrap">
                        <a href="{{ route('laporan.cetakPeminjaman') }}"
                           class="btn btn-sm btn-primary" target="_blank">
                            📄 Semua Periode
                        </a>
                        <a href="{{ route('laporan.cetakPeminjaman', ['bulan' => $bulan, 'tahun' => $tahun]) }}"
            ...
            <div class="col-md-6">
                <div class="rounded-3 p-3 h-100 d-flex flex-column" style="border:1px solid #fef3c7;background:#fffbeb">
                    ...
                    <div class="d-flex gap-2 mt-auto flex-wrap">
                        <a href="{{ route('laporan.cetakPengembalian') }}"
                           class="btn btn-sm btn-warning text-dark" target="_blank">
                            📄 Semua Periode
                        </a>
                        <a href="{{ route('laporan.cetakPengembalian', ['bulan' => $bulan, 'tahun' => $tahun]) }}"
    ...
    <div class="col-sm-6 col-xl-3">
        <div class="card stat-card h-100">
            <div class="card-body d-flex align-items-center gap-3 p-3">
                ...
                <div>
                    <div class="fs-3 fw-bold" style="color:#0f766e">{{ $data['totalPeminjaman'] }}</div>
    ...
    <div class="col-sm-6 col-xl-3">
        <div class="card stat-card h-100">
            <div class="card-body d-flex align-items-center gap-3 p-3">
                ...
                <div>
                    <div class="fs-3 fw-bold text-danger">{{ $data['totalTerlambat'] }}</div>
    ...
    <div class="col-sm-6 col-xl-3">
        <div class="card stat-card h-100">
            <div class="card-body d-flex align-items-center gap-3 p-3">
                ...
                <div>
                    <div class="fw-bold text-danger" style="font-size:1rem">Rp {{ number_format($data['belumBayar'], 0, ',', '.') }}</div>
    ...
    <div class="col-sm-6 col-xl-3">
        <div class="card stat-card h-100">
            <div class="card-body d-flex align-items-center gap-3 p-3">
                ...
                <div>
                    <div class="fw-bold text-success" style="font-size:1rem">Rp {{ number_format($data['sudahBayar'], 0, ',', '.') }}</div>
...
<div class="rounded-3 shadow-sm mb-4 p-4 d-flex align-items-center justify-content-between"
    ...
    <div>
        <div class="fw-bold fs-5">
            💰 Total Denda {{ $daftarBulan[str_pad($bulan,2,'0',STR_PAD_LEFT)] }} {{ $tahun }}
        </div>
        <div class="small" style="opacity:.75">{{ $data['jumlahKasus'] }} kasus keterlambatan</div>
    </div>
    <div class="fs-3 fw-bold">
        Rp {{ number_format($data['totalDenda'], 0, ',', '.') }}
    </div>
</div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($data['bukuPopuler'] as $i => $b)
                            <tr>
                                <td class="text-muted small">{{ $i + 1 }}</td>
                                <td>
                                    <div class="fw-semibold small">{{ $b->judul }}</div>
                                    <div class="text-muted" style="font-size:.72rem">✍️ {{ $b->pengarang }}</div>
                                </td>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            @forelse($data['anggotaAktif'] as $i => $a)
                            <tr>
                                <td class="text-muted small">{{ $i + 1 }}</td>
                                <td>
                                    <div class="fw-semibold small">{{ $a->name }}</div>
                                    <div class="text-muted" style="font-size:.72rem">{{ $a->no_anggota ?? $a->email }}</div>
                                </td>
                                <td class="text-center">
                                    <span class="badge rounded-pill" style="background:#ede9fe;color:#5b21b6">
                                        {{ $a->peminjaman_count }}x
                                    </span>
                                </td>
                            </tr>
                            @empty
...
<div class="card table-card">
    <div class="card-header py-3">
        <h6 class="mb-0 fw-bold">
            💸 Detail Denda — {{ $daftarBulan[str_pad($bulan,2,'0',STR_PAD_LEFT)] }} {{ $tahun }}
        </h6>
    </div>
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    @forelse($data['detailDenda'] as $i => $p)
                    <tr>
                        <td class="text-muted small">{{ $i + 1 }}</td>
                        <td class="small fw-semibold">{{ $p->user->name }}</td>
                        <td class="small text-truncate" style="max-width:140px">{{ $p->buku->judul }}</td>
                        <td class="small">{{ $p->tanggal_kembali->format('d/m/Y') }}</td>
                        <td class="small">{{ $p->tanggal_dikembalikan ? $p->tanggal_dikembalikan->format('d/m/Y') : '—' }}</td>
                        <td class="small text-danger fw-semibold">⏰ {{ $p->jumlah_hari_terlambat }} hari</td>
                        <td class="small fw-semibold text-danger">Rp {{ number_format($p->total_denda, 0, ',', '.') }}</td>
                        <td>
